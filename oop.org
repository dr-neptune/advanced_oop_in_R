...
In R6, the name is mainly used to set the class attribute so the objects can interact with s3 polymorphism. It is the return value of R6Class we use to create objects.
...
VectorStack <- R6Class("VectorStack",
                       ...
                       public = list(
                           top = function() private$elements[1],
                           pop = function() {
                           ...
                           is_empty = function() length(private$elements == 0)))
#+END_SRC

Besides class name, we also have private and public. These are used to define attributes of the class, either values stored in objects or methods we can call on the objects. The difference is that attributes in the public list can be accessed anywhere while attributes in the private list can only be accessed in methods you define for the class. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
(stack <- VectorStack$new())

# we can modify the print method for our vectorstack 
VectorStack <- R6Class("VectorStack",
                       private = list(elements = NULL),
                       public = list(
                           top = function() private$elements[1],
...
# we can access the public attributes of the stack object 
...
If we want to set attributes of objects when constructing them, we need to do a little more work than in S4. To be able to initialize objects with default values, we need to explicitly write a public function called initialize. 
...
#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
# example to see the difference between public and private attributes
...
# you cannot create new attributes to R6 objects just by assignment like in s3 
a$z <- "foo"
#+END_SRC
...
This is achieved through the active argument to R6Class. Functions in the active list should only take one argument, value. 
...
Overall, writing methods and class hierarchies is done with much simpler code in R6 than in both S3 and S4. 
...
An important difference of R6 is that objects have a state and can be modified. 

The first thing we must know is that values set in the definition of private and public lists are shared between objects of a class. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
A <- R6Class("A", public = list(x = 1:5))
B <- R6Class("B", public = list(x = 1:5, a = A$new()))

x <- B$new(); y <- B$new()
x$x; y$x

# if we modify the vector in x we see that the vector in y doesn't change 
(x$x <- 1:3)
y$x

# if we mod the vector in the nested A object, we see state is shared
x$a$x
y$a$x
...
If you want each object of a class to contain distinct objects of an R6 class, then you can create objects in the initialize function instead of in the public or private lists. 

#+BEGIN_SRC R :post round-tbl[:colnames yes](*this*)
B <- R6Class("B",
             public = list(x = 1:5,
                           a = NULL,
                           initialize = function() self$a <- A$new()))
...
Since assigning from one variable to another just creates another reference to the same object, we need another way of creating an effective copy. This is done with the clone method that all R6 objects automatically implement. 
...
The default cloning is shallow. It makes a copy of the object, but if the object contains a reference to an R6 class, then the clone will contain a reference to the same object. 
...
# deep = TRUE gives us a deep copy with a transitive closure of cloned references with distinct obj
y <- x$clone(deep = TRUE)
x$a$x
y$a$x <- NULL
x$a$x
...
We don't have a mechanism for defining new operators for R6 objects, but we can use the S3 system for this. 
...
# define arithmetic operations on the class using S3 
# the implementation is diff since the accessors are different
Ops.modulus <- function(e1, e2) {
    nx <- ny <- NULL
...
If you make subclasses of an R6 class like modulus, you will get a class attribute that reflects this, so the s3 dispatch mechanism will also work for subclasses in the R6 system. 
